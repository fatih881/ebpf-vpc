...
The motivations behind this redesign are:
- The previous attempt revealing secrets at DockerFile as a text,
...
- Hardware Check : When configuring,a play will make sure kernel suits our needs on XDP/eBPF-related tests.This check only contains;
  ...
  - Since the result of this integration test will cover everything we need end-to-end,(e.g,checking related kernel options are enabled),no necessary implementations found necessary on eBPF.
...
- Secret Management : Since secret management is bullet points of technical debts of previous attempt,related PR to this RFC uses cloud-init for secret management. See  [How security risks are prevented?](#to-provide-security--isolation)
...
- Cloud-init must close all firewalls but loopback after run. Dependencies (e.g., DNS queries) must be allowed.
  - This service must have `Before=network-pre.target` order.
  - We assume services like CloudFlared and GitHub Actions use outbound-initiated connections. Ensure initial outbound connections are allowed before final lockdown via systemd ordering or cloud-init hooks to avoid race conditions.
