...
- GitHub Action Listeners must be run on bare metal since we need to use NIC directly for integration tests (also, tests must leave a clean environment even in failing), control plane must be using a Kubernetes distribution which will be suit our needs.
...
- GitHub Actions Listener : Since we need to work on features of NIC's, GitHub Actions cannot run integration tests on a virtualized environment. Other test types can be run on containers for keeping the environment clean.
- Secret Management : Ansible Vault is used at baking process,and HashiCorp vault or GitHub Secrets can be used for injecting secrets via cloud-init at deploy.
